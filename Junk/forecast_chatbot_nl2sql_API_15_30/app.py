...
from forecast_revenue_model import detect_intent as detect_forecast_intent, handle_prompt as handle_forecast
import os
import io
import tempfile
import wave
...
# Intent Identifier: Determine if user is asking for forecasting or data query
def identify_intent(question: str) -> str:
    """
    Identifies user intent: 'forecast' (REVENUE ONLY), 'nl2sql'
    
    Routes to Python forecast model for revenue forecasting (when "forecast" or "predict" + "revenue" appears).
    Load forecasting queries use NL2SQL on forecasted_load_kwh column.
    """
    q_lower = question.lower()
    
    # === REVENUE FORECASTING ===
    # If user mentions "forecast" or "predict" with "revenue", route to forecast model
    # This handles: "forecast revenue", "show revenue forecast", "revenue forecast of date X", etc.
    
    has_forecast_word = any(word in q_lower for word in ["forecast", "predict", "projection"])
    has_revenue = "revenue" in q_lower
    
    if has_forecast_word and has_revenue:
        return "forecast"
    
    # === LOAD FORECASTING OR ANY OTHER QUERY ===
    # Even if user says "forecast load" or "predict load", use NL2SQL
    # because we have forecasted_load_kwh column in the database
    if "load" in q_lower or "kwh" in q_lower or "forecasted" in q_lower:
        return "nl2sql"
    
    # Default to nl2sql for all other queries
    return "nl2sql"
...
async def ask(request: Request):
    ...
    # Identify if user is asking for forecasting or data query
    intent = identify_intent(question)
    
    # ROUTE 1: FORECAST INTENT - Use Python forecast model
    if intent == "forecast":
        try:
            forecast_result = handle_forecast(question)
            
            if "error" in forecast_result:
                return JSONResponse(status_code=400, content={"error": forecast_result["error"], "route": "forecast"})
            
            # Format forecast results - convert from list of dicts to list of lists for frontend
            forecast_data = forecast_result.get("forecast", [])
            # Convert each dict to a list: [Datetime_parsed, Predicted_Revenue]
            forecast_rows = [
                [
                    str(item.get("Datetime_parsed", "")),
                    round(item.get("Predicted_Revenue", 0), 2)
                ]
            ...
            return {
                "question": question,
                "route": "forecast",  # Show which route was taken
                "forecast_intent": intent,
                "horizon": forecast_result.get("horizon", 0),
                "frequency": forecast_result.get("freq", "M"),
                "output_type": "forecast",
                "result": forecast_rows,  # Now list of lists instead of dicts
                "ncols": 2 if forecast_rows else 0,
                "columns": ["Datetime_parsed", "Predicted_Revenue"],
                "summary": f"Forecasted revenue for next {forecast_result.get('horizon', 12)} periods ({forecast_result.get('freq', 'M')})",
                "include_table": True
            }
        except Exception as e:
            return JSONResponse(status_code=500, content={"error": f"Forecast failed: {str(e)}", "route": "forecast"})
    
    # ROUTE 2: NL2SQL INTENT - Use natural language to SQL conversion
    ...
    response = {
        "question": question,
        "route": "nl2sql",  # Show which route was taken
...
function renderChat() {
  ...
  for (const m of history) {
    ...
    if (m.role === 'user') {
      ...
      // Display route (forecast or nl2sql)
      let headerText = 'Assistant';
      if (m.meta && m.meta.route) {
        const routeLabel = m.meta.route === 'forecast' ? '📊 Forecast Model' : '🔍 NL2SQL Query';
        headerText = `${headerText} (${routeLabel})`;
      } else if (m.meta && m.meta.output_type) {
        headerText = `${headerText} (${m.meta.output_type})`;
      }
      ...
      if (m.meta && m.meta.output_type === 'forecast' && m.meta.summary) {
        const p = document.createElement('div');
        p.style.marginTop = '8px';
        p.style.fontWeight = 'bold';
        p.textContent = m.meta.summary;
