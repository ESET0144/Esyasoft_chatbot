...
function renderChartFromData(data) {
  ...
  if (ncols >= 3) {
    ...
    const valueIndex = first.length - 1;
  ...
  // If too many points, downsample to maxPoints
  const maxPoints = 200;
  if (labels.length > maxPoints) {
    ...
    for (let i = 0; i < labels.length; i += step) {
      downLabels.push(labels[i]);
      downValues.push(values[i]);
    }
    labels = downLabels;
    values = downValues;
  }

  // ensure canvas has fixed height so chart won't stretch
  ...
  chartInstance = new Chart(ctx, {
    ...
    options: {
      responsive: true,
      maintainAspectRatio: false, // we control canvas height via CSS and style
      plugins: {
